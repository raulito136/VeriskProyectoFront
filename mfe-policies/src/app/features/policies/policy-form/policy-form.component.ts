...
export class PolicyFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (idParam && idParam !== 'new') {
      ...
      this.form.addControl('status', this.fb.control('ACTIVE', [Validators.required]));
  ...
  private loadPolicy(id: number): void {
    ...
    this.service.getPolicyById(id).subscribe({
      ...
      error: () => {
        this.globalErrorMessages = ['Failed to load policy details.'];
        this.isLoading = false;
      }
    });
  }
  ...
  onSubmit(): void {
    ...
    const requestObservable = this.isEditMode && this.policyId
